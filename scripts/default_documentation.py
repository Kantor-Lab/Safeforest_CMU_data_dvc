...
def parse_args():
    ...
    parser.add_argument(
        "--move-files",
        action="store_true",
        help="Move files into raw, processed_1, and processed_2",
    )
...
def rename_to_raw(folder):
    ...
    ub.util_path.ensuredir(folder, mode=0o0755)
    shutil.move(temp, raw)
    ub.util_path.ensuredir(proc_1, mode=0o0755)
    ub.util_path.ensuredir(proc_2, mode=0o0755)


def move_files(folder, search_string="site_*/*/collect_*"):
    files = Path(folder).glob(search_string)
    [rename_to_raw(f) for f in files]
